...
Step 5  PRICE LOOKUP
    Entry: exact close on trigger date.
    Exit : exact close on trigger date.
"""
...
def _nifty_trailing_peak_exit(
    nifty: pd.DataFrame,
    start: pd.Timestamp,
    threshold_pct: float,
) -> tuple[Optional[pd.Timestamp], float, float, float]:
    """
    TRAILING-PEAK SELL (used by the short-term engine).

    Track the running PEAK close from `start` (the Fall leg / sell-phase entry
    date) onward and SELL on the first close that falls `threshold_pct` BELOW
    that running peak.  The scan is UNBOUNDED — it deliberately runs past the
    sell-phase exit date if the drop has not yet happened, i.e. "if the fall's
    exit date comes, don't sell — keep waiting till the fall."

    Returns (trigger_date|None, base_close, close_at_trigger, peak_close).
    trigger_date=None means the drop never occurred within the data.
    """
    base = _nifty_near(nifty, pd.Timestamp(start), "forward")
    if base is None:
        return None, 0.0, 0.0, 0.0
    if threshold_pct <= 0.0:
        return pd.Timestamp(start), base, base, base
    ...
    for dt, c in sub["close"].items():
        ...
        if c > peak:
            peak = c
        if c <= peak * (1.0 - threshold_pct / 100.0):
            return dt, base, c, peak

    # Never dropped enough — caller decides the fallback (sell at last date).
    return None, base, float(sub["close"].iloc[-1]), peak
...
def _entry_price(stock: pd.DataFrame, trigger: pd.Timestamp, phase_end: pd.Timestamp
    ...
    """Entry: exact trigger date close, else walk FORWARD within phase."""
    sub = stock[(stock.index >= trigger) & (stock.index <= phase_end)]
    if sub.empty:
        return None, None
...
def _exit_price(stock: pd.DataFrame, trigger: pd.Timestamp, phase_start: pd.Timestamp
    ...
    """Exit: exact trigger date close, else walk BACKWARD within phase."""
    sub = stock[(stock.index >= phase_start) & (stock.index <= trigger)]
    if sub.empty:
        return None, None
...
def _annualized_vol(close: pd.Series) -> float:
    """Annualised standard volatility from a series of daily closing prices.

    NOTE: All closes in the window are used for ranking.

    Method (NATURAL LOG, ln):
      • daily return  r_t = ln(P_t / P_{t-1})
      • volatility    = population_std(r_t) * sqrt(252)
                      = sqrt( (1/N) * sum((r_t - mean_r)²) ) * sqrt(252)

    Uses population std (ddof=0) to be consistent with the downside vol
    formula which also uses N as the denominator.
    Input data is already trading-day-only so no weekend filter is needed.
    """
    if close is None or len(close) < 3:
        return float("nan")
    s = close[close > 0]                           # log needs positive prices
    if len(s) < 3:
        return float("nan")
    logret = np.log(s / s.shift(1)).replace([np.inf, -np.inf], np.nan).dropna()
    if len(logret) < 2:
        return float("nan")
    return float(logret.std(ddof=0) * np.sqrt(252.0))


def _window_vol(stock: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp) -> float:
    """Annualised ln volatility over [start, end].

    Prepends the last close BEFORE `start` so that shift(1) inside
    _annualized_vol produces N log returns from N closes in [start, end],
    not N-1 (which would drop the opening return on the first row).
    """
    if stock is None:
        return float("nan")
    ...
    if sub.empty:
        return float("nan")
    # Prepend one prev close as the base for the first log return
    prev = stock[stock.index < start].tail(1)
    sub_for_vol = pd.concat([prev[["close"]], sub[["close"]]]) if not prev.empty else sub[["close"]]
    return _annualized_vol(sub_for_vol["close"])
...
def _nifty_leg_return(nifty: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp) -> Optional[float]:
    """NIFTY % return between two dates."""
...
def _leg_topn(returns_df: pd.DataFrame, pid: int, top_n: int, sort_by: str,
    ...
    """Top-N (best) or Bottom-N (worst) tickers for one pattern leg → (metrics, set).
    Ranking includes a canonical quartile (rank 1 = best by `sort_by`)."""
    ph = returns_df[returns_df["phase_id"] == pid].dropna(subset=[sort_by])
    if ph.empty:
        return {}, set()
    top = (ph.nsmallest(top_n, sort_by) if selection_side == "bottom"
           else ph.nlargest(top_n, sort_by)).reset_index(drop=True)
    n = len(top)
    metrics = {}
...
def _entry_segment_topn(
    ...
    for tkr, sdf in stock_dict.items():
        epx, _ = _entry_price(sdf, seg_start, seg_end)   # exact close within segment
        xpx, _ = _exit_price(sdf, seg_end, seg_start)    # exact close within segment
    ...
    df = (pd.DataFrame(rows, columns=["ticker", "seg_return", "seg_alpha"])
            .sort_values("seg_alpha", ascending=(selection_side == "bottom"))
            .head(top_n)
...
def compute_window_candidates(
    ...
    vol_pct             : int = 50,
    vol_n               : int = 50,
    selection_side      : str = "top",   # "top" (best) | "bottom" (worst)
    ...
    for li, pid in enumerate(window["phase_ids"]):
        m, s = _leg_topn(returns_df, pid, top_n, sort_by, selection_side)
    ...
    if include_entry_segment:
        ...
        if seg_end <= seg_start:
            ...
            basis = "buy-start → buy-phase-end (entry triggered at start)"
        seg_m, seg_s, nifty_seg = _entry_segment_topn(stock_dict, nifty_df, seg_start, seg_end, top_n, selection_side)
    ...
    if (not detail_df.empty) and vol_mode in ("low", "high"):
        ...
        valid = detail_df.dropna(subset=["window_vol"])
        if len(valid) >= 2:
            keep_n = max(1, int(vol_n))
            asc = (vol_mode == "low")          # low → calmest first
            keep_ids = (valid.sort_values("window_vol", ascending=asc)
                             .head(keep_n)["ticker"].tolist())
            kept = detail_df[detail_df["ticker"].isin(keep_ids)]
            if not kept.empty:
                detail_df = kept.reset_index(drop=True)
                vol_basis = (f"{vol_mode} {len(keep_ids)} calmest/wildest of {len(valid)} "
                             f"over {v_start:%d-%b-%Y}→{v_end:%d-%b-%Y}")

    if not detail_df.empty:
        detail_df = detail_df.sort_values(
            "mean_alpha", ascending=(selection_side == "bottom")).reset_index(drop=True)
...
def compute_trade_phase_ranks(
    ...
    date (buy date) to the NIFTY exit-trigger date (sell date).
    ...
    for widx, meta in win_meta.items():
        ...
        stock_rows = []
        for tkr, sdf in stock_dict.items():
            # Entry: first price on/after the buy-trigger, within the buy phase
            epx, edate = _entry_price(sdf, entry_t, buy_end)
            if epx is None or epx <= 0:
                continue
            # Exit: last price on/before the sell-trigger, not before entry
...
def compute_window_stock_ranks(
    ...
    whole window — i.e. a stock must have a valid price based return in EVERY
    pattern leg.
    ...
    *,
    ...
    for win in windows:
        ...
        candidates, cand_detail, cmeta = compute_window_candidates(
            ...
            vol_mode=vol_mode, vol_pct=vol_pct, vol_n=vol_n, selection_side=selection_side,
        )

        # ── NIFTY exit trigger — TRAILING PEAK, UNBOUNDED ─────────────────────
        # SELL only when NIFTY falls `exit_threshold_pct` BELOW the running peak
        # measured from the sell-phase (Fall) entry date.  We deliberately do
        # NOT sell at the Fall-phase exit date — the scan keeps waiting past it
        # until the drop happens.  If it never happens within the data, fall
        # back to the last available NIFTY date (still holding to the end).
        exit_tdate, n_sell_base, n_at_exit, n_sell_peak = _nifty_trailing_peak_exit(
            nifty_df, sell_start, exit_threshold_pct)
        exit_hit = exit_tdate is not None
        if exit_tdate is None:
            _tail = nifty_df[nifty_df.index >= sell_start]
            if not _tail.empty:
                exit_tdate = _tail.index[-1]
                n_at_exit  = float(_tail["close"].iloc[-1])
            else:
                exit_tdate = sell_end
        ...
        for tkr in candidates:
            ...
            per_trade_rows.append({
                ...
                # Exit NIFTY details (trailing-peak stop)
                "nifty_sell_base"          : round(n_sell_base, 2) if n_sell_base else None,
                "nifty_peak"               : round(n_sell_peak, 2) if n_sell_peak else None,
                "nifty_exit_trigger_level" : round(n_sell_peak * (1 - exit_threshold_pct/100), 2) if n_sell_peak else None,
    ...
    for tkr, grp in per_trade_df.groupby("ticker"):
        ...
        # True per-stock Max Drawdown: peak-to-trough on the stock's own equity
        # curve (compounding its trade returns in chronological order, seeded at
        # the starting capital). NOT the worst single-trade return — that could
        # even be positive for an all-winning stock, which is meaningless as a
        # draw-down. Always measured against the running peak.
        _g = grp.sort_values("entry_date") if "entry_date" in grp.columns else grp
        _eq = 1.0
        _peak = 1.0
        _mdd = 0.0
        for _rp in pd.to_numeric(_g["return_pct"], errors="coerce").dropna().astype(float):
            _eq *= (1.0 + _rp / 100.0)
            if _eq > _peak:
                _peak = _eq
            if _peak > 0:
