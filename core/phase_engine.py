...
==============================
Computes per-phase returns for every stock.
  Entry: close on exact entry_date
  Exit : close on exact exit_date
...
def _nifty_return_per_phase(nifty_df: pd.DataFrame, phases: pd.DataFrame) -> dict[int, float]:
    """Compute NIFTY % return for each phase."""
    out = {}
    if nifty_df.empty:
        return out
...
def compute_all_phase_returns(
    ...
    # ── Stock Lookup per (phase, ticker) ───────────────────────────
    #   ENTRY = close on FIRST available date on/after entry_date (within phase).
    #   EXIT  = close on LAST available date on/before exit_date (within phase).
    rows = []
    for _, ph in phases.iterrows():
        pid    = int(ph["phase_id"])
