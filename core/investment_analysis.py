...
def compute_investment_analysis(
    ...
    # Seed the curve with the starting capital at t0 so it is *continuous*
    # (begins at the initial capital, before any window has settled). This makes
    # the plotted/exported equity curve and any draw-down measured off it honest:
    # a dip below the starting capital in the very first window is now visible.
    _start_dt = (window_table["entry_date"].dropna().min()
                 if "entry_date" in window_table.columns else pd.NaT)
    _seed = pd.DataFrame([{"exit_date": _start_dt, "equity_inr": float(initial_capital)}])
    equity_curve = pd.concat([_seed, equity_curve], ignore_index=True)
...
def _compute_metrics(df: pd.DataFrame, window_table: pd.DataFrame, initial: float) -> dict:
    ...
    # ── Elapsed span (first entry → last exit), used to annualise ─────────────
    ...
    if len(entry_dates) and len(exit_dates):
        span_days = (exit_dates.max() - entry_dates.min()).days
        years = (span_days / 365.25) if span_days > 0 else np.nan

    # ── CAGR — compound annual growth rate of the portfolio equity ────────────
    #   CAGR = (final / initial) ** (1 / years) − 1
    #   • needs a positive elapsed span and positive starting capital
    #   • a full wipe-out (final ≤ 0) is reported as −100 %, not N/A, since the
    #     compound formula is undefined for a non-positive ending equity
    cagr = np.nan
    if (not np.isnan(years)) and years > 0 and initial > 0:
        cagr = -100.0 if final_equity <= 0 else \
               ((final_equity / initial) ** (1.0 / years) - 1.0) * 100.0

    # ── Max drawdown — peak-to-trough on the COMPOUNDED return path ────────────
    #   Draw-down is a geometric concept, so it is ALWAYS measured on a
    #   reinvested (compounded) equity curve, regardless of the `reinvest`
    #   display setting. This keeps MDD within (-100%, 0].
    #
    #   We deliberately do NOT measure draw-down off the non-reinvested ₹ ledger:
    #   that ledger is additive (it re-deploys the full starting capital every
    #   window and sums the P&L), so a long losing streak can drive it below zero
    #   and yield a meaningless draw-down worse than -100%. The compounded path
    #   below cannot do that — a window's mean return can't fall below -100%,
    #   so every (1 + r) factor stays ≥ 0. The curve is seeded at 1.0 (the
    #   starting capital) so an early dip below the starting level is captured.
    wret = window_table["window_return_pct"].astype(float).values / 100.0
    mult = np.concatenate(([1.0], np.cumprod(1.0 + wret))) if len(wret) else np.array([1.0])
    mdd = 0.0
    peak = -np.inf
    for v in mult:
        if v > peak:
        ...
        if peak > 0:
            ...
            if dd < mdd:
                mdd = dd
    mdd_pct = mdd * 100.0   # in (-100%, 0]

    # ── CAR / MDD (Calmar) = CAGR / |Max Drawdown| ────────────────────────────
    #   Undefined when there is no draw-down (mdd_pct == 0) or no valid CAGR.
