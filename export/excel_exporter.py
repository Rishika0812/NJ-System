...
def _write_portfolio_summary(workbook, per_trade_df: pd.DataFrame, summary_df: pd.DataFrame,
    ...
    _lowvol_lbl = (f"On — keep {config.get('lowvol_n', 200)} calmest stocks"
                   if _lowvol_on else "Off")
    _winvol_mode = config.get("winvol_mode", "off")
    if _winvol_mode == "low":
        _winvol_lbl = f"Low (calmest) — keep {config.get('winvol_n', 50)} stocks"
    elif _winvol_mode == "high":
        _winvol_lbl = f"High (wildest) — keep {config.get('winvol_n', 50)} stocks"
    else:
        _winvol_lbl = "Off"
    ...
    config_rows = [
        ...
        ("Selection Side", "Top-N (best) & common"
         if config.get("selection_side", "top") == "top" else "Bottom-N (worst) & common"),
        ...
        ("Exact Trigger Mode", "Yes — Sell at n% fall; Buy at n% recovery (separate dates)"
         if config.get("exact_trigger_mode") else "No — Sell & next Buy on same date (recovery)"),
    ]
